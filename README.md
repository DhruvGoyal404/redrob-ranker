...
```
candidates.jsonl
      │  src/parse.py        stream-parse + normalize (handles .gz)
      ▼
 [ trap / validity gate ]    src/traps.py
      │  honeypots → bottom (internal-consistency rules; tier-0, DQ-filter safe)
      │  keyword-stuffers → severe penalty (nontech title + unbacked AI skills)
      ▼
 [ hybrid retrieval ]        src/retrieve.py
      │  BM25  ⊕  dense cosine (bge-small, precomputed)  → RRF (k=60) → shortlist
      ▼
 [ signal scorer ]           src/score.py
      │  7 weighted base components  ×  availability/location/disqualifier modifiers
      ▼
 [ grounded reasoning ]      src/reasoning.py
      │  facts + confidence tag + honest concern + counterfactual (borderline)
      ▼
